...
│  ┌──────────────┐    ┌──────────────┐   ┌───────────┐│
│  │  frontend    │    │   backend    │   │    db     ││
│  │  (nginx)     │───▶│  (Tomcat 9   │──▶│ (MySQL 8) ││
│  │  포트 8080   │ JS │   + WAR)     │JDBC│ 포트 3306 ││
│  │              │    │  포트 8081   │   │           ││
│  └──────────────┘    └──────────────┘   └───────────┘│
│                                              │        │
│                                              ▼        │
...
| 컨테이너 | 이미지 | 외부 포트 | 역할 |
|---------|--------|----------|------|
| frontend | `nginx:alpine` | 8080 | 정적 HTML/CSS/JS 서빙 |
| backend  | `tomcat:9-jdk17` | 8081 | WAR 배포, REST API 제공 |
| db       | `mysql:8.0` | 3306 (디버깅용) | 데이터 영속화 (볼륨 마운트) |

**통신 구조:**
- 프론트엔드는 브라우저에서 `http://localhost:8081/api/...` 로 fetch 호출한다.
- 백엔드는 컨테이너 내부 호스트명 `db:3306`으로 MySQL에 접속한다.
- 백엔드는 모든 응답에 CORS 헤더 (`Access-Control-Allow-Origin: *`)를 포함한다.
...
평균은 정수 점수 리스트에서 계산하며, 빈 리스트일 경우 `null`을 반환하여 등급은 `"-"`로 표시된다.
